...
Extracts structure: state variables, functions, events, modifiers, errors, inheritance.
"""
import re
...
class Function:
    ...
    params: str
    returns: str
    body_lines: List[Tuple[int, str]]   # [(lineno, text), ...]
    is_constructor: bool
    is_fallback: bool
    is_receive: bool
    raw: str
...
class Contract:
    name: str
    line: int
    kind: str                    # contract, interface, abstract, library
...
def _strip_comments(source: str) -> Tuple[str, List[str]]:
    """Remove // and /* */ comments, preserve line structure."""
    ...
    for line in source.splitlines():
        idx = line.find('//')
        if idx >= 0:
            # Don't strip inside strings — simple heuristic: count quotes before idx
            before = line[:idx]
...
def _find_block_end(lines: List[str], start_line: int) -> int:
    """Given a line with an opening '{', return the line index of the matching '}'."""
    depth = 0
...
_VIS = r'(?:public|private|internal|external)'
_MUT = r'(?:payable|view|pure)'
...
def parse(source: str) -> ParsedFile:
    ...
    lines = raw_lines  # keep original for reporting
    ...
    # Find contract/interface/library blocks
    for cm in _CONTRACT_RE.finditer(clean_source):
        ...
        # Locate block start/end by line number
        ...
        while i < len(contract_lines):
            ...
            # State variables (simple heuristic: line ending in ; not starting with return/emit/etc.)
            sv_skip = re.match(r'^\s*(return|emit|require|revert|assert|if|for|while|else|'
                               r'mapping|address|uint|int|bool|bytes|string|\/\/)', text)
